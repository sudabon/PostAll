...
void main() {
  group('チャネル階層', () {
    ...
    test('同一階層の同名を衝突として扱う', () {
      final channels = [channel(1, name: 'general'), channel(2, name: 'general', parentId: testId(1))];

      expect(hasNameConflict(channels, name: 'general', parentId: null), isTrue);
      expect(hasNameConflict(channels, name: 'general', parentId: null, excludingId: testId(1)), isFalse);
      expect(hasNameConflict(channels, name: 'general', parentId: testId(2)), isFalse);
    });
  });
  ...
  group('日付', () {
    test('ローカルタイムゾーンで日付キーを決める', () {
      ...
      expect(localDateKey(value), '${value.year}-03-0${value.day}'.replaceAll('-030', '-03-0'));
  ...
  group('抜粋', () {
    test('一致箇所を強調し、前後を切り詰める', () {
      final excerpt = buildExcerpt('${'あ' * 100}検索語${'い' * 100}', '検索語', radius: 10);
  ...
  group('SSE', () {
    test('CRLF をまたぐチャンクでも 1 件として組み立てる', () async {
      ...
      final messages = await parseSseStream(Stream.fromIterable(chunks)).toList();
  ...
  group('タイムライン', () {
    ...
    test('重複を除いて束ねる', () {
      ...
      final merged = mergePosts(existing, [post(1, channelId: 'c', body: '更新'), post(2, channelId: 'c')]);
  ...
  group('添付の上限', () {
    test('サイズ・件数・形式を検証する', () {
      expect(
        attachmentRejection(contentType: 'image/png', sizeBytes: 1, alreadyAttached: 0),
        isNull,
      );
      expect(
        attachmentRejection(contentType: 'image/png', sizeBytes: 1, alreadyAttached: 10),
        contains('10 件'),
      );
      expect(
        attachmentRejection(
      ...
      expect(
        attachmentRejection(contentType: 'application/x-sh', sizeBytes: 1, alreadyAttached: 0),
        contains('添付できません'),
      );
    });
