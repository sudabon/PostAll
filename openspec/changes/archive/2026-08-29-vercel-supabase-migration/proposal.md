...
- **BREAKING** 変更通知を SSE から **Supabase Realtime Broadcast** へ置換し、`GET /v1/events/stream` を廃止する。
  ...
  - 新しいクライアントセッションは `after=latest` で現在位置を取得してから表示データを読み直し、過去イベントを `0` から再生しない。`change_events` は30日保持とし、保持範囲外のカーソルには全再取得が必要であることを返す。
...
- 常駐処理をサーバーレスの実行モデルへ適合させる。
  - 起動時の goose マイグレーションを削除し、GitHub Actions の独立した手動ワークフローで適用する。
  - 添付リーパーの常駐 goroutine を Vercel Cron から叩く専用エンドポイントへ置き換える。
  - 絵文字同期 `emoji-sync` を手動マイグレーション後のジョブへ移す。
  - Vercel の Git 自動デプロイを無効にし、本番デプロイも独立した手動 GitHub Actions から実行する。デプロイ側は未適用マイグレーションを検査するだけで、適用は行わない。
...
- `deployment`: コンテナ構成・Nginx リバースプロキシ・Certbot による証明書運用・`pg_bigm` 入り PostgreSQL という要件を、マネージドホスティング（Vercel）とマネージド PostgreSQL（Supabase）を前提とする要件へ全面的に置き換える。TLS と証明書更新はプラットフォームの責務となり、マイグレーションと本番デプロイは分離した手動 GitHub Actions から実行される。
...
- **マイグレーション**: `pg_bigm` 拡張と `gin_bigm_ops` 索引の撤去、PGroonga 拡張と索引の作成、`pg_notify` トリガーの `realtime.send()` 化、`users` の識別子カラム改名、変更イベントの削除済みウォーターマーク追加。既存の再帰 CTE・部分索引・plpgsql トリガーはそのまま Supabase で動く。
- **API 契約（`api/openapi.yaml`）**: `GET /v1/events/stream` を削除。`GET /v1/events` へ後方互換な `after=latest` と任意の `resetRequired` を追加する。`POST /v1/attachments/uploads` の応答（署名付き URL とヘッダ）を Supabase Storage の形に合わせて見直す。生成物（`backend/internal/api/`、`frontend/src/api/schema.d.ts`、`mobile/lib/api/generated/`）が追随する。
...
- **GitHub Actions（`.github/workflows/`）**: 既存 CI は維持。マイグレーション + 絵文字同期と Vercel 本番デプロイを、相互に自動起動しない別々の `workflow_dispatch` ワークフローとして追加する。日次のデータベースエクスポートと 6 時間ごとの添付回収・変更イベント整理は定期実行を維持する。`testcontainers` を使う統合テストは PGroonga 入りイメージへ切り替える。
