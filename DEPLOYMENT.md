...
- [x] GitHub サインイン（事前登録済みメール）が通り、既存ユーザーへ紐付く。
- [x] **未登録**の GitHub アカウントではサインインできない。
- [x] 投稿・編集・論理削除・スレッド返信。
- [x] 添付のアップロードとダウンロード（25 MiB 超が拒否されること）。
- [x] 絵文字リアクションの付与と解除。
- [x] 絵文字画像が表示される（サーバは Storage の `Head` 後に 304、または署名付き URL へ 302 を返す。`fetch` → `blob()` が 302 に追随することの確認）。
- [x] 全文検索（日本語 2 文字、語の途中一致、大文字小文字非依存）。
- [x] 他クライアントの変更が表示中の画面へ反映される。



### 8.2 同期の退避経路

- [x] Realtime を切断した状態（ネットワーク遮断や購読失敗）で、15 秒間隔のポーリングへ退避して変更が反映される。
- [x] iOS は購読失敗中に接続状態が `degraded` と表示され、ポーリング成功では `online` に戻らない。
- [x] 再接続時に指数バックオフで復帰する。
...
- [x] `/v1/*` の存在しないパスが**アプリシェルではなく API のエラー応答**を返す。
...
- [x] `authenticated` の JWT で PostgREST 経由の直接読み書きができない（00013 の lockdown が効いている）。
- [x] 匿名で `postall:events` を購読できない。
